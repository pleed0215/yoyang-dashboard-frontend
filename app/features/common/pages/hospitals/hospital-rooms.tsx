...
import {
  CREATE_HOSPITAL_ROOM_MUTATION,
  RETRIEVE_MY_HOSPITAL_WARDS_AND_ROOMS_QUERY,
  UPDATE_HOSPITAL_ROOM_MUTATION,
  DELETE_HOSPITAL_ROOM_MUTATION,
...
export default function HospitalRoomsPage({}: Route.ComponentProps) {
  ...
  const {
    ...
    refetch,
  } = useQuery<RetrieveMyHospitalWardsAndRoomsQuery>(RETRIEVE_MY_HOSPITAL_WARDS_AND_ROOMS_QUERY, {
  ...
  const wardOptions = useMemo(
    () =>
      wards.map((w: any) => ({
        id: w.id != null ? String(w.id) : w.name,
        name: w.name,
        rooms: w.rooms ?? [],
      })),
    [wards]
  ...
  useEffect(() => {
    ...
    const exists = wardOptions.some(w => w.id === selectedWardId);
    if (!exists) {
      setSelectedWardId('');
    }
  }, [selectedWardId, wardOptions]);
  useEffect(() => {
    const currentWardParam = searchParams.get('ward') ?? '';
    if (selectedWardId) {
      if (currentWardParam === selectedWardId) {
        return;
      }
    ...
    params.delete('ward');
    setSearchParams(params, { preventScrollReset: true });
  }, [selectedWardId, searchParams, setSearchParams]);
      ...
      <Card className="w-full max-w-2xl">
        ...
        <CardContent>
          <div className="mb-4 flex flex-col md:flex-row gap-2 items-center justify-center">
            <label htmlFor="ward-select" className="font-medium">
              병동 선택:
            </label>
            <select
              id="ward-select"
          ...
          <form
            ...
            className="flex gap-2 mb-4 justify-center items-end"
          >
            <LabelInput
              label="병실명"
              {...register('name')}
                      ...
                      {editRoomId === (item.id ?? item.name) ? (
                        <div className="flex gap-2 items-center">
                          ...
                          <Button
                            ...
                            type="button"
                            className="cursor-pointer"
                            onClick={() => handleEditRoom(item.id ?? item.name)}
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          {item.size && (
                            <span className="text-xs text-muted-foreground">({item.size}인실)</span>
                          )}
                        </div>
                      )}
                    </TableCell>
                      ...
                      {editRoomId === (item.id ?? item.name) ? null : (
                        <>
                          <Button
                            ...
                            onClick={() => {
                              setEditRoomId(item.id ?? item.name);
                              setEditRoomName(item.name);
                              setEditRoomSize(item.size ? String(item.size) : '');
                            }}
                          >
                            수정
                          </Button>
